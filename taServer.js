  ...
  */
  if (file.split(".").length === 2) {
    ...
    switch (reqURL.pathname) {
      case "//SYMBOL": {
        let slam = await symbolData();
        client_response(slam, response);
        break;
      }
      case "//IBM/Manuals": {
        await IBM_Manuals();
      ...
      default:
        response.end("<h1>Problems " + reqURL + "</h1>", "utf-8");
    }
  }
  ...
  async function IBM_Manuals() {
    let lines = [];
    const storage = new Storage();
    const [fileObjects] = await storage
      .bucket("triticumarchives")
      .getFiles({ prefix: "IBM" });

    for (let jx = 0; jx < fileObjects.length; jx++) {
      const F = fileObjects[jx];
      lines.push(
        `${F.name}\t${F.metadata.updated}\t${F.metadata.timeCreated}\t${F.metadata.size}\t${F.metadata.generation}\t${F.metadata.bucket}\r\n`
      );
    }
